...
local function eventListener(event, button, x, y)
    if event == "mouse_click" then
        if x >= 15 and y >= 2 and x <= 25 and y <= 4 then
            forceStopped = true
            rednet.broadcast(true, "reactor3Control")
        elseif x >= 2 and y >= 2 and x <= 12 and y <= 4 then
            forceStopped = false
            rednet.broadcast(false, "reactor3Control")
        end
    end
end
...
return {printPage = printPage, eventListener = eventListener, rednetListener = rednetListener, pageName = "Reaktor 3"}